...
# Function to convert input file into loci dictionary, set of loci, filtered fa dictionary from string 
...
def bin_dictionary(gene_freq,num_bins=500,filter_bin_data=True):
    ...
    for i in range(num_bins):
        lower_bound = int(min_freq + i*bin_width)
        upper_bound = int(lower_bound + bin_width)
        ...
        for gene,freq in gene_freq.items():
            if freq >= lower_bound and freq < upper_bound:
                if gene in fa_set:
                    bin_diction[str(lower_bound)+'-'+str(upper_bound)]["fa_genes"].append(gene)
                else:
                    bin_diction[str(lower_bound)+'-'+str(upper_bound)]["non_fa_genes"].append(gene)
        ...
        for outer_key, inner_dict in bin_diction.items():
            for inner_key, inner_value in inner_dict.items():
                if not inner_value and inner_key == 'fa_genes':
                   empty_keys.append((outer_key))
                   break

        filtered_bin_diction = {k:v for k,v in bin_diction.items() if k not in empty_keys}
        return bin_diction,filtered_bin_diction,empty_keys

    else:
        return bin_diction
    
### Generate two populations of 5000 random subnetworks for FA genes
...
def subnetworks(filtered_loci_dictionary,num_iterations=5000,print_subnetwork=False):
    ...
    for i in range(num_iterations):
        ...
        for key,value in filtered_loci_dictionary.items():
            ...
            iteration_fa_genes.add(value[random_index])
            ...
            iteration_nonfa_genes.add(non_fa_values[random_index_nonfa])
            seed1+=1
            seed2+=1
...
def permutation_test(subnetwork_diction,null_subnetwork_diction,num_permutations=10000):
    ...
    for i in range(num_permutations):
        ...
        all_network_keys = list(subnetwork_diction.keys())+list(null_subnetwork_diction.keys())
        swap_labels = [net for net in all_network_keys]
        for index in range(len(all_network_keys)):
            random_index=(seed1*seed2)%len(all_network_keys)
            seed1+=1
            seed2+=1
